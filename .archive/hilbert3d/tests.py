    ...
    def check_from_seed(self, seed, iterations):
        ...
        for nreps in range(1, iterations):
            ...
            for ii in range(nreps):
                s = h.iterate(s)
            arrpath = np.array(h.generatePath(s, distance=1, n=1))
            mins = np.zeros(3)
            maxs = np.zeros(3)
            for ii in range(3):
...
class TestVoxFractalMakeVoxelMethod(unittest.TestCase):
    ...
    pos1 = np.array([0, 0, 0])
    vox1 = h.Voxel(pos1, dirs["x"], dirs["y"], dirs["x"], dirs["y"])
...
class TestEulerAngleGeneration(unittest.TestCase):
    def test_recoverEulerAngles(self):
        for ii in range(100):
            ...
            newpsi, newtheta, newphi = h.getEulerAngles(
...
class TestTextOutput(unittest.TestCase):
    ...
    def test_straightPath(self):
        ...
        for (vtext, voxel) in zip(text, frac.fractal):
            self.assertEqual(
                vtext + "\n", voxel.toText(), "voxel text output incorrect"
            )
            self.assertEqual(
                vtext,
                " ".join(
                    ...
                    + map(str, [voxel.psi, voxel.theta, voxel.phi])
                ),
