...
def make_movie(infile, centre, distance, outfile, clip=100):
    ...
    positions = []
    for ii in range(24):
        angle = (ii + 1) * 3.14159 / 12.0  # a little less than pi
        positions.append(
            [
    ...
    the_frame = the_frame + 120
    bpy.data.scenes[sceneKey].frame_set(the_frame)
    cam_ob.location = [centre[0], centre[1], centre[2] - 0.5 * distance]
    cam_ob.keyframe_insert(data_path="location", frame=the_frame)
    cam_ob.keyframe_insert(data_path="rotation_euler", frame=the_frame)
    ...
    print(
        "Rendering {} frames".format(
            int(the_frame)
            * bpy.data.scenes[sceneKey].render.frame_map_new
...
def make_render(infile, camera_position, camera_aim, outfile, clip=100):
    """Render a frame

    make_render(infile, camera_position, camera_aim, outfile, clip=100)

    args:
        infile: blender file to Render
        camera_position: 3-element list for position
        camera_aim: Point to aim camera
    ...
    point_camera(cam_ob, camera_aim)
...
def assemble_geometry(infile, outfile, units, filepath, placement_dict, ellipse=None):
    ...
    # read_in
    infile = open(os.path.join(filepath, infile))
    names = []
    positions = []
    rotations = []
    for line in infile:
...
def placement_volume(infile, outfile, filepath):
    ...
    for (ii, (name, sz, pos, rot)) in enumerate(
        zip(names, sizes, positions, rotations)
    ):
        if (ii // 100) * 100 == ii:
        ...
        try:
            ...
            print(f"Material not defined for {name.upper()}")
            color = (100, 100, 100)
