...
class Prism(object):
    ...
    def __init__(
        ...
        self.angx = rotation
        self.angy = -np.arctan2(axis[2],
                                (axis[0] ** 2 + axis[1] ** 2) ** 0.5)
        ...
        ry =\
            np.array([[np.cos(self.angy), 0, np.sin(self.angy)],
                      [0, 1, 0],
                      [-np.sin(self.angy), 0, np.cos(self.angy)]])
        rz =\
            np.array([[np.cos(self.angz), -np.sin(self.angz), 0],
        ...
        corners = [0.5*np.array([size[0], size[1], size[2]]),
                   ...
                   0.5*np.array([-size[0], -size[1], size[2]]),
                   0.5*np.array([-size[0], -size[1], -size[2]])]
        self.corners = [self.center + np.dot(self.rotation, corner)
    ...
    def to_text(self):
        ...
        return " ".join(map(str, [self.center[0], self.center[1],
                                  self.center[2], euler[0], euler[1],
                                  euler[2]]))

    def to_plot(self, n=100):
        """
        Prism.to_plot(n)
        ...
        for (axis, sz) in zip([self.axis, self.norm1, self.norm2], self.size):
            a = Arrow3D(self.center, axis*sz + self.center,
    ...
    def does_overlap(self, other):
        ...
        if d > sum_rads:
            ...
            test_axes = [self.axis, self.norm1, self.norm2,
                         other.axis, other.norm1, other.norm2]

            overlaps = [is_separating_axis(ax, self.corners, other.corners)
                        for ax in test_axes]
    ...
    def contains_point(self, point):
        ...
        """
        overlaps = [is_separating_axis(ax, self.corners, point)
...
class PrismList(object):
    ...
    def get_prism_grid_indices(self, prism: Prism) -> Set[int]:
        ...
        indices = set([self._xyz_to_index(np.array([i_, j_, k_]))
                      for i_ in ii for j_ in jj for k_ in kk])
        return indices
    ...
    def _ijk_to_index(self, ijk: Union[np.array, List]) -> int:
        ...
        """
        return int(ijk[0] + self.divisions * ijk[1] + self.divisions ** 2 * ijk[2])  # NOQA
...
def random_point_in_ball(rad=1):
    """
...
def generate_non_overlapping_prisms(
    ...
    attempts = 0
    while n_prisms < n:
        if ((n_prisms // 10000) * 10000 == n_prisms):
            sys.stderr.write("Generating prism " + str(n_prisms) + " of " +
                             str(n) + "\n")

        if attempts >= threshold:
            sys.stderr.write("Too many failed prism placements, stopped after "
                             + str(n_prisms) + " placements.\n")
            break
        this_prism = new_prism()

        if prisms.append(this_prism) is True:
            if verbose is True and (n_prisms//20) * 20 == n_prisms:
                print("Placing prism {0} of {1}".format(n_prisms, n))
            attempts = 0
            n_prisms += 1
        else:
            attempts += 1
