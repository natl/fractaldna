...
class TestPrismClass(unittest.TestCase):
    ...
    def test_prism_properties(self):
        for ii in range(0, 10000):
            ...
            self.assertAlmostEqual(np.dot(axis, prism.axis), 1, 7,
                                   "Axis is mal-transformed")
            self.assertAlmostEqual(np.dot(prism.norm1, prism.axis), 0, 7,
                                   "Axis and normal are not orthogonal")
            self.assertAlmostEqual(np.dot(prism.norm1, prism.norm2), 0, 7,
                                   "Normals are not orthogonal")

    def test_contains_generated_points(self):
        for ii in range(0, 10):
            c = (ii+1) * 2 * (np.random.rand(3) - .5)
            s = 10*np.random.rand(3)
            ax = np.random.rand(3) - 0.5
            r = 2 * np.pi * np.random.rand()
            prism = rp.Prism(c, s, ax, r)
            for jj in range(0, 100):
                self.assertTrue(prism.contains_point(prism.get_point()),
    ...
    def test_contains_no_excluded_points(self):
        for ii in range(0, 10):
            ...
            for jj in range(0, 100):
                ...
                self.assertFalse(prism.contains_point(p),
    ...
    def test_overlapping_cylinders(self):
        ...
        for ii in range(0, 10000):
            ...
            self.assertTrue(prism1.does_overlap(prism2),
                            "Overlapping prisms do not overlap\n" +
                            "iter.: " + str(ii) + "\n" +
                            "intersection: " + repr(intersect) + "\n" +
                            "position1: " + repr(point1) + "\n" +
                            "size1: " + repr(size1) + "\n" +
                            "axis1: " + repr(ax1) + "\n" +
                            "position2: " + repr(point2) + "\n" +
                            "size2: " + repr(size2) + "\n" +
                            "axis2: " + repr(ax2) + "\n\n" +
                            "(intsct-p1)/ax1:" +
    ...
    def plot_two_prisms(prism1, prism2):
        ...
        for (axis, sz) in zip([prism1.axis, prism1.norm1, prism1.norm2],
                              prism1.size):
            a = rp.Arrow3D(prism1.center, axis*sz + prism1.center,
        ...
        for (axis, sz) in zip([prism2.axis, prism2.norm1, prism2.norm2],
                              prism2.size):
            a = rp.Arrow3D(prism2.center, axis*sz + prism2.center,
