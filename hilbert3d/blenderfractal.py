...
def blenderifyFractal(vf):
    ...
    for filepath in filepaths:
        with bpy.data.libraries.load(filepath, link=True) as (data_from,
                                                              data_to):
            data_to.groups = data_from.groups

    mdict = {h.Voxel.types["turn"]: "dnaturn",
    ...
    for voxel in vf.fractal:
        ...
        bpy.ops.object.group_instance_add(group=shape,
                                          location=tuple(voxel.pos),
                                          rotation=(voxel.psi, voxel.theta,
                                                    voxel.phi))
